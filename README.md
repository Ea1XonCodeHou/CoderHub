...
![Tavily](https://img.shields.io/badge/Tavily-Search-00B4D8)
![WebSocket](https://img.shields.io/badge/WebSocket-实时私信-6366f1)
...
目前仓库代码已完成 `v1.0.6` 版本的核心功能合并，系统整体运行稳定。

- **实时私信系统**：基于原生 WebSocket 实现双向实时通讯，信息中心、私信咨询入口已上线。
- **AI 联网搜索**：集成 Tavily Search API，深度检索与联网可同时开启，本地知识库优先。
...
#### v1.0.6 (2026-03-17) - 实时私信系统

**💬 私信聊天功能上线**

- **WebSocket 实时通讯**：基于原生 Spring WebSocket（无 STOMP/SockJS 依赖），用户间消息毫秒级双向推送；支持断线自动重连，前端 5 秒后重试
- **信息中心**：导航栏新增「信息中心」入口，未读消息红色角标实时更新；左侧联系人列表 + 右侧对话区的经典 IM 布局，兼容移动端折叠
- **私信咨询**：文章作者卡片新增「私信咨询」按钮，一键跳转并自动打开与该作者的会话
- **CoderHub 助手**：用户首次打开信息中心，系统自动创建欢迎会话并推送欢迎语，永久置顶于联系人列表顶部
- **在线状态**：基于 WebSocket SESSIONS Map 实时判断对方是否在线，点击会话时动态查询

#### v1.0.5 (2026-03-14) - AI 联网搜索

**🌐 Tavily 联网搜索集成**

- 集成 **Tavily Search API**，AI 回答时可补充互联网实时内容；深度检索与联网搜索可同时开启
- 优先级设计：本地 RAG 知识库优先，联网结果作为补充，避免互联网内容覆盖平台内容
- 搜索结果卡片化展示，显示来源站点名（`github.com`、`stackoverflow.com` 等）及匹配度
- 后端通过 OkHttp 直接调用 Tavily REST API，无额外依赖；API Key 通过环境变量注入
